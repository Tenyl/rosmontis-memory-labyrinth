import { Brain, CheckCircle, Info, PushPin, WarningDiamond } from '@phosphor-icons/react';
  ...
  return (
    <section className="reasoning-board" aria-labelledby="reasoning-board-title">
      ...
      <div className="evidence-columns">
        <EvidenceColumn title="支持证据" tone="support" icon={<CheckCircle size={18} weight="fill" aria-hidden />} records={support} empty="钉选已验证或部分验证的档案以增强假设。" />
        <EvidenceColumn title="冲突证据" tone="conflict" icon={<WarningDiamond size={18} weight="fill" aria-hidden />} records={conflicts} empty="当前钉选证据未检测到直接冲突。" />
      </div>
...
function EvidenceColumn({ title, tone, icon, records, empty }: { title: string; tone: string; icon: React.ReactNode; records: ArchiveRecord[]; empty: string }) {
  return <section className={`evidence-column is-${tone}`}><header>{icon}<h3>{title}</h3><span>{records.length}</span></header>{records.length ? <div>{records.map((record) => <article key={record.id}><span>{record.code}</span><strong>{record.title}</strong><p>{record.summary}</p><footer><PushPin size={13} weight="fill" aria-hidden />可信度 {record.confidence}% · {record.verification}</footer></article>)}</div> : <p className="evidence-empty">{empty}</p>}</section>;
}
