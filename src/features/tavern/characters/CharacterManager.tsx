...
export function CharacterManager() {
  ...
  return (
    <section id="tavern-panel-characters" className="tavern-panel-stack" aria-label="内部角色卡管理">
      <header className="tavern-section-heading"><div><span className="panel-code">CHARACTER & PERSONA / V2</span><h3>角色与身份</h3><p>角色卡决定模型的叙事人格；玩家身份作为独立上下文注入，不会改变本局迷迭香状态。</p></div><div className="tavern-toolbar"><label id="character-import-label" className="terminal-button" htmlFor="character-import-input"><UploadSimple size={16} aria-hidden />导入角色卡</label><input id="character-import-input" className="visually-hidden" type="file" accept=".json,application/json" aria-label="导入 SillyTavern V2 角色卡" onChange={(event) => void importCard(event)} /><button id="character-create-button" className="terminal-button is-primary" type="button" onClick={() => setEditing(null)}><Plus size={16} aria-hidden />新建角色</button></div></header>
