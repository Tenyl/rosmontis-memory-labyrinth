...
function ChipInput({ id, label, value, onChange }: { id: string; label: string; value: string[]; onChange: (value: string[]) => void }) {
  ...
  return <div className="tavern-chip-field"><label htmlFor={id}>{label}</label><div>{value.map((item) => <span key={item}>{item}<button type="button" aria-label={`移除关键词 ${item}`} onClick={() => onChange(value.filter((candidate) => candidate !== item))}><MinusCircle size={14} aria-hidden /></button></span>)}<input id={id} value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={keyDown} onBlur={add} /><button type="button" aria-label={`添加${label}`} onClick={add}><Plus size={14} aria-hidden /></button></div></div>;
}
