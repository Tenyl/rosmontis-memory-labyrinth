...
- 80—99：`berserk`，巨剑伤害乘 2，禁用探针/认知等精细操作，每次攻击结算后扣除稳定性。
- 100：`collapse`，Run 失败。
...
阶段一“破除心防”：允许立柱/破壁、门扉/守望和安抚；目标是把心防降到 0。

阶段二“拥抱与共鸣”：禁用所有伤害动作，只允许哀鸣/共鸣与安抚；目标是把共鸣度升到 100。非法卡片必须显示角色化原因，不得静默无效。
